...
(function (DataType) {
    class Matrix {
        constructor(xLength, yLength) {
            ...
            this._dataArray = Array(xLength * yLength);
        }
        get(x, y) {
    ...
    class TileReuseMatrix extends Matrix {
        constructor(xLength, yLength, elements) {
            super(xLength, yLength);
            this._centerCoord = null;
            this._ifReusableArray = Array(xLength * yLength).fill(false);
        ...
        getIfReuse(x, y) {
            if (x >= 0 && x < this._xLength && y >= 0 && y < this._yLength) {
                ...
                throw RangeError(`out of matrix edge:x=${x},y=${y}`);
        }
        /**
        ...
        setCenter(x, y) {
            ...
            if (Math.abs(this._centerCoord.x - x) >= this._xLength ||
            ...
            let dx = this._centerCoord.x - x, dy = this._centerCoord.y - y;
            let nonReusableList = [];
            this._ifReusableArray.fill(false);
            for (let oldX = 0, newX = oldX + dx; oldX < this._xLength; ++oldX, newX = oldX + dx)
                for (let oldY = 0, newY = oldY + dy; oldY < this._yLength; ++oldY, newY = oldY + dy) {
                    if (newX >= 0 &&
                        newX < this._xLength &&
                        newY >= 0 &&
                        newY < this._yLength) {
            ...
            while (p < this._dataArray.length) {
                while (this._ifReusableArray[p] && p < this._dataArray.length)
                    ++p;
    ...
    DataType.TileReuseMatrix = TileReuseMatrix;
})(DataType || (DataType = {}));
